  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white text-lg font-semibold">Commit History</h3>
        <span className="text-white/60 text-sm">{commits.length} commits</span>
      </div>
                ...
                <div className="pt-3 border-t border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {commit.files_changed.slice(0, 3).map((file, index) => (
                      <span
                        key={index}
                        className="px-2 py-1 bg-white/10 rounded-md text-white/70 text-xs font-mono"
                      >
                        {file}
                      </span>
                    ))}
                    {commit.files_changed.length > 3 && (
                      <span className="px-2 py-1 bg-white/10 rounded-md text-white/70 text-xs">
